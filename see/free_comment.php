...
if($num==1){
	$sql = "insert into free_comment(id, detail, seq_num) value('$id','$detail','$seq_re');";
	$result = mysqli_query($conn, $sql);
	...
	if($result===false){
		...
		$num = 0;
?>
<meta http-equiv="refresh" content="0;URL='/see/freeboardsee.php?seq=<?php echo $seq_re;?>'"><!--
<?php	
	}
}
$sql="select *from free_comment where seq_num=$seq_num";
$result=mysqli_query($conn,$sql);
?>
<html lang="ko">
<head>
	...
	<body>
		...
		?>
		<form name="writeform" action="free_comment.php" method="post" class="comment_input">
			<p>
