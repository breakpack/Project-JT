...
$sql = "select *from documentboard where seq = $see_num";
...
if(!isset($title)){
	echo "<script>alert('삭제된 글입니다.'); location.href='/board/document.php'</script>";
}
$sql_see_count = "update documentboard set see_count = see_count + 1 where seq = $see_num";
...
<head>
	...
	<body>
		...
		<div class='detail'>
			...
			<li class='file'>첨부파일 : 
				...
				if(in_array($ext, $ext_array)){
					echo "<img src=/typing/img/{$filename} width='500' />";
				}else{ ?>
					<a class="file_href" href='/typing/img/<?php echo $filename ?>'><?php echo $filename ?></a>
				<?php
				}
				?>
				</li>
		...
		<form action="documentde.php" method="post">
		...
		<hr class=line/>
		<?php include("document_comment.php"); ?>
	</body>
</html>